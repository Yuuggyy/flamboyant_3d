...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU FLAMBOYANT (gastronomique, buffet 40$, lounge)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Entrees Gourmandes','Entrees raffinees et tapas','🥗',1,true),
('Plats Principaux','Cuisine gastronomique locale et francaise','🍖',2,true),
('Pizzas et Pates','Pizzas au four et pates maison','🍕',3,true),
('Desserts','Patisseries et douceurs maison','🍰',4,true),
('Bar et Cocktails','Cocktails signature, vins, bieres','🍹',5,true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Veloute de Legumes','Potage du jour, creme fraiche, croutons',8.00,(SELECT id FROM categories WHERE nom='Entrees Gourmandes'),true,1),
('Carpaccio de Boeuf','Roquette, parmesan, capres, huile d olive',14.00,(SELECT id FROM categories WHERE nom='Entrees Gourmandes'),true,2),
('Foie Gras Poele','Pain d epices, compotee d oignons',22.00,(SELECT id FROM categories WHERE nom='Entrees Gourmandes'),true,3),
('Cocktail de Crevettes','Crevettes froides, sauce cocktail maison',16.00,(SELECT id FROM categories WHERE nom='Entrees Gourmandes'),true,4),
('Salade Chevre Chaud','Chevre, miel, noix, roquette',14.00,(SELECT id FROM categories WHERE nom='Entrees Gourmandes'),true,5),
('Buffet Complet Diner','Entrees + plat + dessert - formule soir',40.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,1),
('Entrecote Sauce Poivre','300g, sauce poivre vert, gratin dauphinois',32.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,2),
('Poulet Roti aux Herbes','Demi-poulet, legumes rotis de saison',22.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,3),
('Poisson du Jour','Selon arrivage, sauce au choix',24.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,4),
('Magret de Canard','Sauce aux fruits rouges, pommes sarladaises',35.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,5),
('Risotto aux Champignons','Champignons sauvages, parmesan, beurre noisette',20.00,(SELECT id FROM categories WHERE nom='Plats Principaux'),true,6),
('Pizza Margherita','Tomate, mozzarella, basilic frais',18.00,(SELECT id FROM categories WHERE nom='Pizzas et Pates'),true,1),
('Pizza 4 Fromages',NULL,22.00,(SELECT id FROM categories WHERE nom='Pizzas et Pates'),true,2),
('Spaghetti Carbonara','Lardons, creme, parmesan, oeuf',18.00,(SELECT id FROM categories WHERE nom='Pizzas et Pates'),true,3),
('Tagliatelles Fruits de Mer','Crevettes, calamars, sauce cremeuse',24.00,(SELECT id FROM categories WHERE nom='Pizzas et Pates'),true,4),
('Tiramisu Maison','Mascarpone, cafe, amaretto',12.00,(SELECT id FROM categories WHERE nom='Desserts'),true,1),
('Fondant au Chocolat','Coulant, boule de glace vanille',12.00,(SELECT id FROM categories WHERE nom='Desserts'),true,2),
('Panna Cotta','Coulis de fruits rouges',10.00,(SELECT id FROM categories WHERE nom='Desserts'),true,3),
('Creme Brulee','Vanille de Madagascar',11.00,(SELECT id FROM categories WHERE nom='Desserts'),true,4),
('Profiteroles','Glace vanille, sauce chocolat chaude',12.00,(SELECT id FROM categories WHERE nom='Desserts'),true,5),
('Cocktail Signature Flamboyant','Recette exclusive Flamboyant',12.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,1),
('Vin Rouge (verre)','Selection du sommelier',9.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,2),
('Champagne (flute)','Moet et Chandon ou equivalent',20.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,3),
('Biere Importee 33cl',NULL,6.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,4),
('Jus de Fruit Frais',NULL,6.00,(SELECT id FROM categories WHERE nom='Bar et Cocktails'),true,5)
ON CONFLICT DO NOTHING;
